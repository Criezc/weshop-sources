...
<?php
    ...
    $query = mysqli_query($koneksi, "SELECT barang.*, kategori.kategori FROM barang JOIN kategori ON barang.kategori_id=kategori.kategori_id $where ORDER BY barang.barang_id DESC LIMIT $mulai_dari, $data_per_halaman");
        ...
        if(mysqli_num_rows($query)== 0) {
            echo "<h3> There is no item inside item for now</h3>";
        }else{
            ...
            while($row=mysqli_fetch_assoc($query)) {
                ...
                echo "<tr>
                        ...
                        <td class='tengah'>
                            <a class='tombol-edit' href='".BASE_URL. "index.php?page=my_profile&module=barang&action=form&barang_id=$row[barang_id]'>Edit</a>
                            <a class='tombol-edit' href='".BASE_URL. "module/barang/action.php?button=Delete&barang_id=$row[barang_id]'>Delete</a>
                        </td>
                    </tr>";
